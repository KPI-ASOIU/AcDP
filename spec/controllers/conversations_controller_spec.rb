      ...
      @request.env["HTTP_REFERER"] = "http://#{request.host}/users/#{@user.id}"
		...
		context "with valid message attributes" do 
			it "creates a new conversation" do 
				expect{ 
					post :create, conversation: @conversation_attr, participants_ids: "#{@current_user.id} #{@user.id}", message: @message_attr
		...
		context "with invalid message attributes" do 
			it "doesn't save message" do
				expect{ 
					post :create, conversation: @conversation_attr, participants_ids: "#{@current_user.id} #{@user.id}", message: @message_invalid_attr
